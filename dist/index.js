...
var PLAYER_CLIENT_ORDER = [
  "ANDROID",
  "IOS",
  "TVHTML5_SIMPLY_EMBEDDED_PLAYER",
  "WEB"
];
var FAILURE_CACHE = /* @__PURE__ */ new Map();
var FAILURE_TTL_MS = 30 * 1e3;
...
async function playerWithFallback(videoId, hl, gl) {
  ...
  for (const name of PLAYER_CLIENT_ORDER) {
    try {
      ...
      if (data && data.streamingData) {
        ...
        if (formats.length > 0) {
          return { data, client: name, playability };
        }
      }
  ...
  if (bestFailure) {
    ...
    const err3 = new Error(bestFailure.error);
    err3.code = bestFailure.code;
    err3.status = bestFailure.status;
    err3.detail = bestFailure.reason;
    throw err3;
  }
  const failObj = {
  ...
  const err2 = new Error(failObj.error);
  err2.code = failObj.code;
  err2.status = failObj.status;
  err2.detail = failObj.detail;
  throw err2;
}
...
function normalizeVideoRenderer(vr) {
  ...
  const title = vr.title && (vr.title.simpleText || vr.title.runs && vr.title.runs[0] && vr.title.runs[0].text) || null;
  const channel = vr.ownerText && vr.ownerText.runs && vr.ownerText.runs[0] && vr.ownerText.runs[0].text || vr.longBylineText && vr.longBylineText.runs && vr.longBylineText.runs[0] && vr.longBylineText.runs[0].text || vr.shortBylineText && vr.shortBylineText.runs && vr.shortBylineText.runs[0] && vr.shortBylineText.runs[0].text || null;
  const channelId = vr.ownerText && vr.ownerText.runs && vr.ownerText.runs[0] && vr.ownerText.runs[0].navigationEndpoint?.browseEndpoint?.browseId || null;
...
var PARAM_PATTERN = /^[a-zA-Z0-9_-]{2,10}$/;
...
async function handleProxy(url, request, env) {
  ...
  async function fetchUpstream(targetUrl, currentClientName) {
    const clientUA = request.headers.get("User-Agent") || CLIENTS[currentClientName] && CLIENTS[currentClientName].userAgent || CLIENTS.ANDROID.userAgent;
